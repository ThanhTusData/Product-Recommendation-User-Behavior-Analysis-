...
if __name__ == "__main__":
    ...
    recommender = MF_KNN_Recommender(
        latent_factors=100,  # Reduced to train faster
        steps=200,           # Reduced to train faster
        k=5,
        alpha=0.001,
        beta=0.01
    )
    ...
    try:
        ...
        print(f"Training a new model... ({str(e)})")
    ...
    for i, user_id in enumerate(test_users):
        print(f"\n{i+1}. Recommendations for user {user_id}:")
        try:
    ...
    for i, user_id in enumerate(test_users):
        print(f"\n{i+1}. Diversified recommendations for user {user_id}:")
        try:
    ...
    if plot_path:
        print(f"Embedding plot saved at: {plot_path}")
    
    # Calculate recommendation coverage (using a smaller sample)
